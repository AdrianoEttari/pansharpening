...
class Attention_UNet_superres(nn.Module):
    ...
    def forward(self, lr_img, magnification_factor):

        # LR ENCODER
        x = self.LR_encoder(lr_img)
 
        # UPSAMPLE LR IMAGE
        try:
            upsampled_lr_img = F.interpolate(x, scale_factor=magnification_factor, mode='bicubic')
        except:
            upsampled_lr_img = F.interpolate(x.to('cpu'), scale_factor=magnification_factor, mode='bicubic').to(self.device)

        x = self.conv_upsampled_lr_img(upsampled_lr_img)
